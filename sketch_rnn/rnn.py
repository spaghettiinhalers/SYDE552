...
# damage p weights to simulate cognitive deficit!!
def damage_random(weights, p_random, name='random_damaged_weights'):
  shape = weights.get_shape().as_list()
  num_weights = np.prod(shape)
  num_damage = int(p_random * num_weights)

  # create randomized binary mask
  mask_base = np.ones(num_weights, dtype=np.float32)
  mask_base[:num_damage] = 0
  np.random.shuffle(mask_base)
  mask = tf.constant(mask_base.reshape(shape), dtype=tf.float32, name=name)
...
class LSTMCell(tf.nn.rnn_cell.RNNCell):
  ...
  def __call__(self, x, state, scope=None):
    with tf.variable_scope(scope or type(self).__name__):
      ...
      w_xh = damage_random(w_xh, self.p_random)
      w_hh = damage_random(w_hh, self.p_random)
