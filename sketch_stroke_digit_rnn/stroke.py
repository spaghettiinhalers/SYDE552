...
# STROKE SIMULATION STARTS HERE!!!!!!

def wallerian_func(x):
    return 8.0070 * (np.exp(0.1386 * x) - 1)
...
for model_version in ['1', '2', '3', '4', '5']:
    ...
    for i in range(10):
        ...
        correct, time = test_model(model, version)
        correct_confidences.append(correct)
        times.append(time)

        damage_fas(model, 0.0938, 0.0, 0.0) # initial stroke: 9.38% of connections blocked
        correct, time = test_model(model, version)
        correct_confidences.append(correct)
        times.append(time)

        # wallerian damage over a year
        for month in range(12):
            month += 1
            print("month", month)

            if month == 1:
                p_wallerian = wallerian_func(1)/100
            else:
                p_wallerian  = (wallerian_func(month) - wallerian_func(month - 1)) / (100 - wallerian_func(month - 1))

            p_block = p_wallerian * 0.3
            p_reflect = p_wallerian * 0.45
            p_filter = p_wallerian * 0.2

            damage_fas(model, p_block, p_reflect, p_filter)

            correct, time = test_model(model, version)
            correct_confidences.append(correct)
            times.append(time)

        ccs.append(correct_confidences)
        tts.append(times)
